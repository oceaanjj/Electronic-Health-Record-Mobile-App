...
const ADLScreen = ({ onBack }: any) => {
  ...
  const [formData, setFormData] = useState({
    mobility: '', hygiene: '', toileting: '', feeding: '', 
    hydration: '', sleep_pattern: '', pain_level: ''
  });
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      if (hasContent) {
        try { await checkADLAlerts(adlId, { ...formData }); } 
        catch (e) { console.error("CDSS Real-time Error:", e); }
      }
    }, 1000); 
  ...
  const handleCDSSPress = async () => {
    ...
    try {
      const result = await saveADLAssessment({ patient_id: selectedPatient.id, ...formData });
      const id = result.id || result.adl_id;
      if (id) {
        setAdlId(id);
        setIsAdpieActive(true); 
      }
    } catch (e) { Alert.alert("Error", "Could not initiate clinical support."); }
  };
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <ScrollView keyboardShouldPersistTaps="handled" style={styles.container} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.row}>
            <View style={{ flex: 1, marginRight: 10 }}>
              ...
              <View style={styles.inputBox}>
                <Text style={styles.inputText}>{getFormattedAdmissionDate()}</Text>
              </View>
            </View>
            <View style={{ flex: 1 }}>
              ...
              <View style={styles.inputBox}>
                ...
                <Icon name="arrow-drop-down" size={24} color={THEME_GREEN} style={{ position: 'absolute', right: 10 }} />
              </View>
            </View>
          </View>
        </View>
        ...
        <ADLInputCard label="MOBILITY" value={formData.mobility} disabled={!selectedPatient} alertText={alerts.mobility_alert} onChangeText={(t) => setFormData({...formData, mobility: t})} />
        <ADLInputCard label="HYGIENE" value={formData.hygiene} disabled={!selectedPatient} alertText={alerts.hygiene_alert} onChangeText={(t) => setFormData({...formData, hygiene: t})} />
        <ADLInputCard label="TOILETING" value={formData.toileting} disabled={!selectedPatient} alertText={alerts.toileting_alert} onChangeText={(t) => setFormData({...formData, toileting: t})} />
        <ADLInputCard label="FEEDING" value={formData.feeding} disabled={!selectedPatient} alertText={alerts.feeding_alert} onChangeText={(t) => setFormData({...formData, feeding: t})} />
        <ADLInputCard label="HYDRATION" value={formData.hydration} disabled={!selectedPatient} alertText={alerts.hydration_alert} onChangeText={(t) => setFormData({...formData, hydration: t})} />
        <ADLInputCard label="SLEEP PATTERN" value={formData.sleep_pattern} disabled={!selectedPatient} alertText={alerts.sleep_pattern_alert} onChangeText={(t) => setFormData({...formData, sleep_pattern: t})} />
        <ADLInputCard label="PAIN LEVEL" value={formData.pain_level} disabled={!selectedPatient} alertText={alerts.pain_level_alert} onChangeText={(t) => setFormData({...formData, pain_level: t})} />
...
const styles = StyleSheet.create({
  ...
  inputBox: { height: 48, borderRadius: 25, borderWidth: 1, borderColor: '#F2F2F2', paddingHorizontal: 20, justifyContent: 'center', backgroundColor: '#fff', marginBottom: 15 },
  ...
  searchBar: { borderRadius: 25, paddingHorizontal: 20, height: 48, borderWidth: 1, borderColor: '#F2F2F2', marginBottom: 15 },
  dropdown: { backgroundColor: '#fff', borderRadius: 10, borderWidth: 1, borderColor: '#eee', elevation: 3, position: 'absolute', top: 75, left: 0, right: 0, zIndex: 99 },
