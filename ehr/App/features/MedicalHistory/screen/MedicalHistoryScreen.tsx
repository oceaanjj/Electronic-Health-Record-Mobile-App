  ...
  const handleNext = async () => {
    if (step < steps.length - 1) {
      ...
      try {
        ...
        Alert.alert("Success", "Medical History components saved successfully.");
        onBack();
      } catch (error: any) {
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      ...
      >
        ...
        {Object.keys(formData[steps[step].key as keyof typeof formData]).map((field) => (
          <HistoryInputCard 
            key={`${steps[step].key}-${field}`}
            label={field.replace('_', ' ').charAt(0).toUpperCase() + field.replace('_', ' ').slice(1)} 
            value={(formData[steps[step].key as keyof typeof formData] as any)[field]}
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 28, color: THEME_GREEN, fontWeight: '600', fontStyle: 'italic', fontFamily: 'serif' },
  ...
  searchBar: { borderRadius: 25, paddingHorizontal: 20, height: 48, borderWidth: 1, borderColor: '#F2F2F2', justifyContent: 'center', backgroundColor: '#fff' },
  searchInput: { fontSize: 14, color: '#333' },
  dropdown: { position: 'absolute', left: 0, right: 0, top: 56, backgroundColor: '#fff', borderRadius: 8, marginTop: 5, borderWidth: 1, borderColor: '#eee', elevation: 8, zIndex: 9999, maxHeight: 220 },
  ...
  stepHeader: { backgroundColor: LIGHT_GREEN_BG, paddingVertical: 10, borderRadius: 25, alignItems: 'center', marginBottom: 20 },
