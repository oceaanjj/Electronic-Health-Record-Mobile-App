...
import { StyleSheet, View, Text, ScrollView, SafeAreaView, TouchableOpacity, TextInput, Pressable, Alert } from 'react-native';
...
const LAB_TESTS = [
  ...
  "Platelets (×10⁹/L)", "MCV (fL)", "MCH (pg)", "MCHC (g/dL)", 
  "RDW (%)", "Neutrophils (%)", "Lymphocytes (%)", "Monocytes (%)", 
...
const LabValuesScreen = ({ onBack }: any) => {
  ...
  useEffect(() => {
    ...
    if (!selectedPatientId || !labId || !result.trim() || !normalRange.trim()) return;
    ...
    const timer = setTimeout(async () => {
      ...
      } catch (e) { console.error("Lab CDSS Error:", e); }
    }, 1000);
  ...
  const handleCDSSPress = async () => {
    ...
    } catch (e) { Alert.alert("Error", "Could not initiate nursing process."); }
  };
  ...
  const isBellActive = result.trim() !== '' && 
                       normalRange.trim() !== '' && 
                       currentAlert && 
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView keyboardShouldPersistTaps="handled" style={styles.container} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
           ...
           <TouchableOpacity onPress={() => setShowLabList(!showLabList)}><Icon name="more-vert" size={35} color={THEME_GREEN} /></TouchableOpacity>
        </View>
          ...
          <View style={styles.dropdownOverlay}>
            <ScrollView nestedScrollEnabled={true}>
              {LAB_TESTS.map((test, index) => (
                <TouchableOpacity key={index} style={styles.dropdownItem} onPress={() => { setSelectedTest(test); setShowLabList(false); }}>
                  <Text style={styles.dropdownItemText}>{test}</Text>
                  {selectedTest === test && <Icon name="check" size={16} color={THEME_GREEN} />}
                </TouchableOpacity>
              ))}
            </ScrollView>
          </View>
          ...
          {showDropdown && filteredPatients.length > 0 && (
            <View style={styles.searchDropdown}>
              {filteredPatients.map((p) => (
                <Pressable key={p.id} onPress={() => { setSelectedPatientId(p.id); setSearchText(p.fullName); setShowDropdown(false); }} style={styles.dropItem}><Text>{p.fullName}</Text></Pressable>
              ))}
            </View>
          )}
        </View>
        ...
        <View style={styles.footerRow}>
          ...
          >
            <Icon 
              name={isBellActive ? "notifications-active" : "notifications-none"} 
              size={24} 
          ...
          {selectedTest === "Basophils (%)" ? (
            <View style={styles.buttonGroup}>
              ...
              <TouchableOpacity style={styles.submitBtn} onPress={handleNextOrSave}>
      ...
      <CDSSModal visible={modalVisible} onClose={() => setModalVisible(false)} alertText={currentAlert} />
    </SafeAreaView>
  );
};
...
const styles = StyleSheet.create({
  ...
  searchBar: { borderRadius: 25, paddingHorizontal: 20, height: 48, borderWidth: 1, borderColor: '#F2F2F2', fontSize: 13, color: '#333' },
  searchDropdown: { backgroundColor: '#fff', borderRadius: 10, borderWidth: 1, borderColor: '#eee', position: 'absolute', top: 75, left: 0, right: 0, zIndex: 99, elevation: 3 },
  dropdownOverlay: { position: 'absolute', top: 90, right: 25, width: 220, maxHeight: 300, backgroundColor: '#fff', borderRadius: 15, zIndex: 100, elevation: 5, borderWidth: 1, borderColor: '#E2E8F0' },
  ...
  footerRow: { flexDirection: 'row', alignItems: 'center', marginTop: 10, paddingBottom: 40 },
  buttonGroup: { flex: 1, flexDirection: 'row', marginLeft: 10 },
  bellBtn: { width: 50, height: 50, borderRadius: 25, backgroundColor: '#FFFBEB', borderWidth: 1, borderColor: '#FDE68A', justifyContent: 'center', alignItems: 'center' },
  activeBell: { backgroundColor: '#FFFBEB', borderColor: '#FDE68A' },
  cdssBtn: { flex: 1, height: 50, backgroundColor: '#F3F4F6', borderRadius: 25, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: '#D1D5DB', marginRight: 5 },
  submitBtn: { flex: 1, height: 50, backgroundColor: '#DCFCE7', borderRadius: 25, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: THEME_GREEN, marginLeft: 5 },
  nextBtn: { flex: 1, marginLeft: 15, height: 52, backgroundColor: '#DCFCE7', borderRadius: 26, flexDirection: 'row', justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: THEME_GREEN },
  cdssText: { color: '#6B7280', fontWeight: 'bold' },
  submitText: { color: THEME_GREEN, fontWeight: 'bold', fontSize: 13 },
  nextText: { color: THEME_GREEN, fontWeight: 'bold', fontSize: 14, marginRight: 5 }
});
