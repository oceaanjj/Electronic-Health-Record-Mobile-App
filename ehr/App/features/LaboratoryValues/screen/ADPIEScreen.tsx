...
import { StyleSheet, View, Text, TouchableOpacity, TextInput, ScrollView, SafeAreaView, Alert } from 'react-native';
...
const ADPIEScreen = ({ onBack, labId, patientName }: any) => {
  ...
  const handleNext = async () => {
    if (!text.trim()) return Alert.alert("Required", `Please document the ${STEPS[currentIdx].label}.`);
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.patientSection}>
        ...
        <View style={styles.patientDisplay}>
          <Text style={styles.patientNameText}>{patientName || "Loading..."}</Text>
        </View>
      </View>
      ...
      <View style={styles.stepperContainer}>
        ...
        <View style={[styles.progressLineActive, { width: `${(currentIdx / (STEPS.length - 1)) * 100}%` }]} />
      ...
      >
        ...
        >
          <Text style={[styles.viewBtnText, !alert && { color: '#999' }]}>VIEW</Text>
          <Icon name="play-arrow" size={14} color={alert ? "#10B981" : "#999"} />
        </TouchableOpacity>
      </LinearGradient>
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#fff', paddingHorizontal: 25, marginBottom: 20 },
  ...
  circle: { width: 36, height: 36, borderRadius: 18, justifyContent: 'center', alignItems: 'center', zIndex: 3 },
