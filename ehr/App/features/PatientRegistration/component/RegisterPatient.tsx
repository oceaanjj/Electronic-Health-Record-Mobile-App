...
const RegisterPatient: React.FC<Props> = ({ onBack }) => {
  ...
  const [contacts, setContacts] = useState([{ name: '', relationship: '', number: '' }]);

  useEffect(() => {
    const today = new Date();
    setDisplayDate(today.toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }));
  }, []);
  ...
  const isFormValid = () => {
    const { first_name, last_name, birthdate, age, sex, address, religion, ethnicity, chief_complaints, room_no, bed_no } = form;
    if (!first_name || !last_name || !birthdate || !age || !sex || !address || !religion || !ethnicity || !chief_complaints || !room_no || !bed_no) {
        ...
        {step === 1 ? (
            ...
            <View style={styles.row}>
              <View style={{ width: '48%' }}>
                ...
                <TouchableOpacity style={styles.input} onPress={() => setShowPicker(true)}>
                  <Text style={{ color: form.birthdate ? '#333' : '#EBEBEB' }}>{form.birthdate || "Select Date"}</Text>
                </TouchableOpacity>
              </View>
              <View style={{ width: '48%' }}>
                <Text style={styles.inputLabel}>Age*</Text>
                <View style={styles.readOnlyBox}>
                  <Text style={styles.readOnlyText}>{calculatedAge || "--"}</Text>
                </View>
              </View>
            </View>

            {showPicker && (
              <DateTimePicker value={dateValue} mode="date" display="default" maximumDate={new Date()} onChange={handleDateChange} />
            )}

            <View style={styles.inputGroup}>
                <Text style={styles.inputLabel}>Sex *</Text>
                <View style={styles.genderContainer}>
                  <TouchableOpacity style={[styles.genderBtn, form.sex==='Male'&&styles.genderBtnActive]} onPress={()=>setForm({...form, sex:'Male'})}>
                    <Text style={form.sex==='Male'?styles.genderTextActive:styles.genderText}>Male</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.genderBtn, form.sex==='Female'&&styles.genderBtnActive]} onPress={()=>setForm({...form, sex:'Female'})}>
                    <Text style={form.sex==='Female'?styles.genderTextActive:styles.genderText}>Female</Text>
                  </TouchableOpacity>
                </View>
            </View>
          ...
          <View>
            <View style={styles.rowBetween}>
              ...
              <TouchableOpacity onPress={() => setContacts([...contacts, {name:'', relationship:'', number:''}])} style={styles.addIconCircle}><Text style={styles.addPlusText}>+</Text></TouchableOpacity>
            </View>
            ...
            {contacts.map((contact, index) => (
              <View key={index} style={styles.contactWrapper}>
                <View style={styles.rowBetween}>
                    ...
                    {contacts.length > 1 && (
                        <TouchableOpacity onPress={() => {setIndexToDelete(index); setAlertVisible(true);}} style={styles.removeCircle}><Text style={styles.removeText}>✕</Text></TouchableOpacity>
                    )}
                </View>
                <View style={styles.inputGroup}>
            ...
            <TouchableOpacity onPress={() => setStep(1)} style={styles.backBtn}><Text style={styles.backBtnText}>Back to Details</Text></TouchableOpacity>
          </View>
        )}
      </ScrollView>
...
const styles = StyleSheet.create({
  ...
  readOnlyBox: { borderWidth: 1, borderColor: '#EBEBEB', borderRadius: 10, padding: 12, backgroundColor: '#EBEBEB', height: 48, justifyContent: 'center' },
  ...
  genderContainer: { flexDirection: 'row', borderWidth: 1, borderColor: '#EBEBEB', borderRadius: 10, height: 45, overflow: 'hidden' },
  ...
  contactHeader: { fontSize: 15, fontWeight: 'bold', color: '#1b4332', marginBottom: 10 },
  addIconCircle: { width: 32, height: 32, borderRadius: 16, borderWidth: 1, borderColor: '#2d6a4f', justifyContent: 'center', alignItems: 'center' },
  addPlusText: { color: '#2d6a4f', fontSize: 22 },
  removeCircle: { width: 24, height: 24, borderRadius: 12, borderWidth: 1, borderColor: '#ff4d4d', justifyContent: 'center', alignItems: 'center' },
  removeText: { color: '#ff4d4d', fontSize: 12 },
  backBtn: { marginTop: 20 },
  backBtnText: { textAlign: 'center', color: '#666', textDecorationLine: 'underline' }
});
