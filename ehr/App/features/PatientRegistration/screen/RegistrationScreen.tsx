...
export default function RegistrationForm({ updateField, onBack }: FormProps) {
  ...
  const addContact = () => setEmergencyContacts([...emergencyContacts, 
    { id: Date.now().toString(), name: '', relationship: '', contactNumber: '' }]);

  const removeContact = (id: string) => emergencyContacts.length > 1 && 
    ...
    <View>
        ...
        <View style={styles.formPadding}>
          ...
          <View style={styles.row}>
            ...
            <View style={styles.col}><Text style={styles.label}>Sex</Text>
              <Dropdown style={styles.dropdown} data={genderData} labelField="label" valueField="value" placeholder="Select" onChange={item => updateField('sex', item.value)} />
            </View>
          </View>
          <Text style={styles.label}>Address</Text>
          <TextInput style={styles.input} placeholder="Street, City, Province" onChangeText={(v) => updateField('address', v)} />
          <View style={styles.row}>
            <View style={styles.col}><Text style={styles.label}>Birth Place</Text><TextInput style={styles.input} placeholder="City" /></View>
            <View style={styles.col}><Text style={styles.label}>Ethnicity</Text><TextInput style={styles.input} placeholder="Ethnicity" /></View>
          </View>
          <Text style={styles.label}>Chief of Complaints</Text>
          <TextInput style={[styles.input, styles.textArea]} multiline placeholder="Symptoms..." />
        </View>
      </View>
      ...
      <View style={styles.card}>
        <View style={[styles.cardHeader, styles.headerRow]}>
          ...
          <TouchableOpacity onPress={addContact}><Icon name="library-add" size={24} color="#fff" /></TouchableOpacity>
        </View>
        <View style={styles.formPadding}>
          {emergencyContacts.map((contact, index) => (
            <View key={contact.id} style={index > 0 ? styles.divider : null}>
              ...
              <View style={styles.row}>
                <View style={styles.col}><Text style={styles.label}>Relationship</Text><TextInput style={styles.input} /></View>
                <View style={styles.col}><Text style={styles.label}>Contact Number</Text><TextInput style={styles.input} keyboardType="phone-pad" /></View>
              </View>
            </View>
          ))}
        </View>
      ...
      <View style={styles.buttonRow}>
        ...
        <CustomButton title="SAVE" onPress={() => console.log("Saved")} variant="gradient" />
      </View>
    </View>
  );
}
