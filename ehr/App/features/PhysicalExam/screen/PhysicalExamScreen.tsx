...
import { StyleSheet, View, Text, TouchableOpacity, ScrollView, TextInput, SafeAreaView, Alert, Pressable } from 'react-native';
...
const PhysicalExamScreen: React.FC<PhysicalExamProps> = ({ onBack }) => {
  ...
  const [formData, setFormData] = useState({
    general_appearance: '', skin_condition: '', eye_condition: '', 
    oral_condition: '', cardiovascular: '', abdomen_condition: '', 
  ...
  const handleCDSSPress = async () => {
    if (!selectedPatientId) return Alert.alert("Error", "Select patient first.");
    ...
    <SafeAreaView style={styles.safeArea}>
      <ScrollView keyboardShouldPersistTaps="handled" style={styles.container} showsVerticalScrollIndicator={false}>
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={onBack}><Icon name="more-vert" size={35} color={THEME_GREEN} /></TouchableOpacity>
        </View>
        ...
        <ExamInputCard label="GENERAL APPEARANCE" value={formData.general_appearance} disabled={!selectedPatientId} alertText={backendAlerts.general_appearance_alert} onChangeText={(t) => setFormData({...formData, general_appearance: t})} />
        <ExamInputCard label="SKIN" value={formData.skin_condition} disabled={!selectedPatientId} alertText={backendAlerts.skin_alert} onChangeText={(t) => setFormData({...formData, skin_condition: t})} />
        <ExamInputCard label="EYES" value={formData.eye_condition} disabled={!selectedPatientId} alertText={backendAlerts.eye_alert} onChangeText={(t) => setFormData({...formData, eye_condition: t})} />
        <ExamInputCard label="ORAL CAVITY" value={formData.oral_condition} disabled={!selectedPatientId} alertText={backendAlerts.oral_alert} onChangeText={(t) => setFormData({...formData, oral_condition: t})} />
        <ExamInputCard label="CARDIOVASCULAR" value={formData.cardiovascular} disabled={!selectedPatientId} alertText={backendAlerts.cardiovascular_alert} onChangeText={(t) => setFormData({...formData, cardiovascular: t})} />
        <ExamInputCard label="ABDOMEN" value={formData.abdomen_condition} disabled={!selectedPatientId} alertText={backendAlerts.abdomen_alert} onChangeText={(t) => setFormData({...formData, abdomen_condition: t})} />
        <ExamInputCard label="EXTREMITIES" value={formData.extremities} disabled={!selectedPatientId} alertText={backendAlerts.extremities_alert} onChangeText={(t) => setFormData({...formData, extremities: t})} />
        <ExamInputCard label="NEUROLOGICAL" value={formData.neurological} disabled={!selectedPatientId} alertText={backendAlerts.neurological_alert} onChangeText={(t) => setFormData({...formData, neurological: t})} />
...
const styles = StyleSheet.create({
  ...
  sectionLabel: { fontSize: 12, fontWeight: 'bold', color: THEME_GREEN, marginBottom: 8 },
  searchBar: { borderRadius: 25, paddingHorizontal: 20, height: 48, borderWidth: 1, borderColor: '#F2F2F2' },
  dropdown: { backgroundColor: '#fff', borderRadius: 10, borderWidth: 1, borderColor: '#eee', elevation: 3, position: 'absolute', top: 70, left: 0, right: 0, zIndex: 99 },
  dropItem: { padding: 15, borderBottomWidth: 1, borderBottomColor: '#f9f9f9' },
  banner: { backgroundColor: '#DCFCE7', paddingVertical: 10, borderRadius: 25, alignItems: 'center', marginBottom: 20 },
  ...
  cdssBtn: { flex: 1, backgroundColor: '#F3F4F6', paddingVertical: 15, borderRadius: 25, alignItems: 'center', marginHorizontal: 5, borderWidth: 1, borderColor: '#D1D5DB' },
  submitBtn: { flex: 1, backgroundColor: '#DCFCE7', paddingVertical: 15, borderRadius: 25, alignItems: 'center', marginHorizontal: 5, borderWidth: 1, borderColor: THEME_GREEN },
