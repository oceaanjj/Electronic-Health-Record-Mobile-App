...
const BACKEND_PORT = 8001; // Changed from 8000 to avoid socket conflicts
...
apiClient.interceptors.response.use(
  ...
  error => {
    if (error.response) {
      ...
      console.error('No response from backend. Check if server is running on', host + ':' + BACKEND_PORT);
    } else {
